...
class OrderStatus extends WebService
{
    ...
    {
        $this->setOptions([
            "action" => "api/v1/order_status/" . $reference . "/informations",
        ]);
    ...
    public function getOrderDocuments($references, $type = "waybill")
    {
        ...
        $this->param["type"] = isset($this->documents[$type])
            ? $this->documents[$type]
            : $type;
        $this->param["envoi"] = implode(",", $references);
        $this->setGetParams();
        $this->setOptions(["action" => ""]);
    ...
    private function doStatusRequest()
    {
        ...
        if ($source !== false) {
            ...
            if (count($this->resp_errors_list) == 0) {
                ...
                $this->order_info = [
                    "emcRef" => $this->xpath
                        ->evaluate("/order/emc_reference")
                        ->item(0)->nodeValue,
                    "state" => $this->xpath->evaluate("/order/state")->item(0)
                        ->nodeValue,
                    "opeRef" => $this->xpath
                        ->evaluate("/order/carrier_reference")
                        ->item(0)->nodeValue,
                    "labelAvailable" => (bool) $this->xpath
                        ->evaluate("/order/label_available")
                        ->item(0)->nodeValue,
                    "labelUrl" => $this->xpath
                        ->evaluate("/order/label_url")
                        ->item(0)->nodeValue,
                    "labels" => $labels,
                    "documents" => $documents,
                ];
