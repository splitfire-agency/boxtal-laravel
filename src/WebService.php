...
class WebService
{
     ...
     * @var array
     */
    protected $ssl_check = ["peer" => true, "host" => 2];
    ...
    {
        $this->auth["user"] = config("boxtal.user");
        $this->auth["pass"] = config("boxtal.pass");
        $this->auth["key"] = config("boxtal.key");
        $this->setEnv(config("boxtal.mode", self::ENV_TEST));
    ...
    public function doRequest()
    {
        ...
        if (curl_errno($req) > 0 || curl_error($req)) {
            ...
            $this->resp_errors_list[] = [
                "code" => "curl_" . curl_errno($req),
                "url" => $curl_info["url"],
            ...
            $curl_info["http_code"] != "400" &&
            $curl_info["http_code"] != "401"
        ) {
            $result = false;
            $this->resp_error = true;
            $this->resp_errors_list[] = [
                "code" => "http_error_" . $curl_info["http_code"],
                "url" => $curl_info["url"],
                "message" =>
            ...
            trim($content_type[0]) != "application/xml" &&
            trim($content_type[0]) != "application/x-download"
        ) {
            $result = false;
            $this->resp_error = true;
            $this->resp_errors_list[] = [
    ...
    public function doRequestMulti()
    {
        ...
        foreach ($this->options as $u) {
            ...
            curl_setopt(
                ...
                $u[CURLOPT_SSL_VERIFYPEER]
            );
            curl_setopt(
                $ch[$i],
                CURLOPT_RETURNTRANSFER,
                $u[CURLOPT_RETURNTRANSFER]
            );
        ...
        */
        ...
        foreach ($ch as $k => $c) {
            ...
            if (curl_errno($c) > 0 || curl_error($c)) {
                ...
                $this->resp_errors_list[$k][] = [
                    "code" => "curl_" . curl_errno($c),
                    "url" => $curl_info["url"],
    ...
    public function setOptions($options)
    {
        ...
        $this->options = [
            ...
            CURLOPT_URL =>
                $this->server . $options["action"] . $this->get_params,
        ...
        foreach ($this->get_params as $param) {
            $this->options[] =
                [
                    ...
                    CURLOPT_CAINFO =>
                        dirname(__FILE__) . "/../ca/ca-bundle.crt",
                ] +
                ($this->timeout != null
    ...
    public function parseResponseMulti($documents)
    {
        ...
        foreach ($documents as $document) {
            ...
            $return_wrapper->appendChild(
                $return_xml->importNode($target_node, true)
            );

            if ($this->hasErrorsMulti($this->xpath[$i])) {
                $errors = $this->xpath[$i]->evaluate("/error");
                foreach ($errors as $e => $error) {
                    $this->resp_errors_list[$i][$e] = [
                        "code" => $this->xpath[$i]
                            ->evaluate("code", $error)
                            ->item(0)->nodeValue,
                        "message" => $this->xpath[$i]
    ...
    private function setResponseErrors($xpath = false)
    {
        ...
        foreach ($errors as $e => $error) {
            $this->resp_errors_list[$e] = [
                "code" => $xpath->evaluate("code", $error)->item(0)->nodeValue,
                "message" => $xpath->evaluate("message", $error)->item(0)
                    ->nodeValue,
            ];
        }
    }
    ...
    {
        $envs = [self::ENV_TEST, self::ENV_PRODUCTION];
    ...
    public function setPlatformParams(
        $platform,
        $platform_version,
        $module_version
    ) {
    ...
    {
        return [
            "URL called" => $this->server . $this->param["action"],
            "Params" => $this->param,
        ];
    }
    ...
    public function encryptPassword($string)
    {
        ...
        return base64_encode(
            openssl_encrypt($string, "aes-128-cbc", $key, true, $iv)
        );
    }
