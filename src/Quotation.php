...
class Quotation extends WebService
{
    ...
    public function setProforma($data)
    {
        foreach ($data as $key => $value) {
            ...
            if (
                ((!isset($value["number"]) || $value["number"] <= 0) &&
    ...
    public function setType($type, $dimensions)
    {
        foreach ($dimensions as $d => $data) {
            ...
            if ($type == "palette") {
                $pallet_dim = explode(
                    "x",
                    $this->pallet_dims[$data["palletDims"]]
                );
    ...
    public function getQuotation(
        ...
        $this->setGetParams([]);
        $this->setOptions(["action" => "api/v1/cotation"]);
    ...
    public function getQuotationMulti($multirequest)
    {
        foreach ($multirequest as $quot_info) {
            ...
            foreach ($quot_info["parcels"]["dimensions"] as $d => $data) {
                $params[$quot_info["parcels"]["type"] . "_" . $d . ".poids"] =
                    $data["poids"];
                $params[
                    $quot_info["parcels"]["type"] . "_" . $d . ".longueur"
                ] = $data["longueur"];
                $params[$quot_info["parcels"]["type"] . "_" . $d . ".largeur"] =
                    $data["largeur"];
                $params[$quot_info["parcels"]["type"] . "_" . $d . ".hauteur"] =
                    $data["hauteur"];
            }
        ...
        $this->setGetParamsMulti([]);
        $this->setOptionsMulti(["action" => "api/v1/cotation"]);
    ...
    public function getOffers($only_com = false, $multi = false, $i = 0)
    {
        ...
        foreach ($offers as $o => $offer) {
            ...
            if (!$only_com || ($only_com && $offer_mode == "COM")) {
                ...
                $informations = $xpath->query(
                    "./mandatory_informations/parameter",
                    $offer
                );
                $mand_infos = [];
                foreach ($informations as $mandatory) {
                    $arr_key = $xpath->query("./code", $mandatory)->item(0)
                        ->nodeValue;
                    $mand_infos[$arr_key] = [];
                    $mandatory_childs = $xpath->query("*", $mandatory);
                    foreach ($mandatory_childs as $mandatory_child) {
                        $mand_infos[$arr_key][
                            $mandatory_child->nodeName
                        ] = trim($mandatory_child->nodeValue);
                ...
                foreach ($options_xpath as $option) {
                    $code_option = $xpath->query("./code", $option)->item(0)
                        ->nodeValue;
                    $options[$code_option] = [
                        "name" => $xpath->query("./name", $option)->item(0)
                            ->nodeValue,
                    ];
                    $description = $xpath
                    ...
                    if (isset($description)) {
                        $options[$code_option]["description"] =
                            $description->nodeValue;
                    }
                    $options[$code_option]["parameters"] = [];
                    ...
                    foreach ($parameters as $parameter) {
                        $param_code = $xpath
                            ...
                            ->item(0);
                        $param_label = $xpath
                            ->query("./label", $parameter)
                            ->item(0);
                        $param_type = $xpath
                            ->query("./type", $parameter)
                            ->item(0);
                        $options[$code_option]["parameters"][
                            $param_code->nodeValue
                        ] = [
                        ...
                        foreach ($nodes as $node) {
                            ...
                            if ($node->nodeName == "enum") {
                                $values = [];
                                $enum = $xpath
                                    ->query("./enum", $param_type)
                                    ->item(0);
                                $param_options = $xpath->query(
                                    "./value",
                                    $enum
                                );
                                foreach ($param_options as $param_option) {
                                    $param_option_id = $xpath
                                        ->query("./id", $param_option)
                                        ->item(0)->nodeValue;
                                    $param_option_label = $xpath
                                        ->query("./label", $param_option)
                                        ->item(0)->nodeValue;
                ...
                $charact_detail = $xpath
                    ->evaluate("./characteristics", $offer)
                    ->item(0)->childNodes;
                $charact_array = [];
                ...
                $return_values[$o] = [
                    ...
                    "operator" => [
                        "code" => $xpath
                            ->query("./operator/code", $offer)
                            ->item(0)->nodeValue,
                        "label" => $xpath
                            ->query("./operator/label", $offer)
                            ->item(0)->nodeValue,
                        "logo" => $xpath
                            ->query("./operator/logo", $offer)
                            ->item(0)->nodeValue,
                    ],
                    "service" => [
                        "code" => $xpath
                            ->query("./service/code", $offer)
                            ->item(0)->nodeValue,
                        "label" => $xpath
                            ->query("./service/label", $offer)
                            ->item(0)->nodeValue,
                    ],
                    "price" => [
                        "currency" => $xpath
                            ->query("./price/currency", $offer)
                            ->item(0)->nodeValue,
                        "tax-exclusive" => $xpath
                            ->query("./price/tax-exclusive", $offer)
                            ->item(0)->nodeValue,
                        "tax-inclusive" => $xpath
                            ->query("./price/tax-inclusive", $offer)
                            ->item(0)->nodeValue,
                    ],
                    "collection" => [
                        "type" => $xpath
                            ->query("./collection/type/code", $offer)
                            ->item(0)->nodeValue,
                        "date" => $xpath
                            ->query("./collection/date", $offer)
                            ->item(0)->nodeValue,
                        "label" => $xpath
                            ->query("./collection/type/label", $offer)
                            ->item(0)->nodeValue,
                    ],
                    "delivery" => [
                        "type" => $xpath
                            ->query("./delivery/type/code", $offer)
                            ->item(0)->nodeValue,
                        "date" => $xpath
                            ->query("./delivery/date", $offer)
                            ->item(0)->nodeValue,
                        "label" => $xpath
                            ->query("./delivery/type/label", $offer)
                            ->item(0)->nodeValue,
                    ],
                ...
                if ($xpath->evaluate("boolean(./insurance)", $offer)) {
                    $return_values[$o]["insurance"] = [
                        "currency" => $xpath
                            ->query("./insurance/currency", $offer)
                            ->item(0)->nodeValue,
                        "tax-exclusive" => $xpath
                            ->query("./insurance/tax-exclusive", $offer)
                            ->item(0)->nodeValue,
                        "tax-inclusive" => $xpath
                            ->query("./insurance/tax-inclusive", $offer)
                            ->item(0)->nodeValue,
                    ];
    ...
    private function getOrderInfos()
    {
        ...
        $this->order["url"] = $this->xpath
            ->query("./url", $offer)
            ->item(0)->nodeValue;
        $this->order["mode"] = $this->xpath
            ->query("./mode", $offer)
            ->item(0)->nodeValue;
        $this->order["offer"]["operator"]["code"] = $this->xpath
            ->query("./operator/code", $offer)
            ->item(0)->nodeValue;
        $this->order["offer"]["operator"]["label"] = $this->xpath
            ->query("./operator/label", $offer)
            ->item(0)->nodeValue;
        $this->order["offer"]["operator"]["logo"] = $this->xpath
            ->query("./operator/logo", $offer)
            ->item(0)->nodeValue;
        $this->order["service"]["code"] = $this->xpath
            ->query("./service/code", $offer)
            ->item(0)->nodeValue;
        $this->order["service"]["label"] = $this->xpath
            ->query("./service/label", $offer)
            ->item(0)->nodeValue;
        $this->order["price"]["currency"] = $this->xpath
            ->query("./service/code", $offer)
            ->item(0)->nodeValue;
        $this->order["price"]["tax-exclusive"] = $this->xpath
            ->query("./price/tax-exclusive", $offer)
            ->item(0)->nodeValue;
        $this->order["price"]["tax-inclusive"] = $this->xpath
            ->query("./price/tax-inclusive", $offer)
            ->item(0)->nodeValue;
        $this->order["collection"]["code"] = $this->xpath
            ->query("./collection/type/code", $offer)
            ->item(0)->nodeValue;
        $this->order["collection"]["type_label"] = $this->xpath
            ->query("./collection/type/label", $offer)
            ->item(0)->nodeValue;
        $this->order["collection"]["date"] = $this->xpath
            ->query("./collection/date", $offer)
            ->item(0)->nodeValue;
        ...
        $this->order["collection"]["label"] = $this->xpath
            ->query("./collection/label", $offer)
            ->item(0)->nodeValue;
        $this->order["delivery"]["code"] = $this->xpath
            ->query("./delivery/type/code", $offer)
            ->item(0)->nodeValue;
        $this->order["delivery"]["type_label"] = $this->xpath
            ->query("./delivery/type/label", $offer)
            ->item(0)->nodeValue;
        $this->order["delivery"]["date"] = $this->xpath
            ->query("./delivery/date", $offer)
            ->item(0)->nodeValue;
        ...
        $this->order["delivery"]["label"] = $this->xpath
            ->query("./delivery/label", $offer)
            ->item(0)->nodeValue;
    ...
    public function makeOrder(
        ...
        if (isset($additionalParams["reason"]) && $additionalParams["reason"]) {
            $additionalParams["raison"] = array_search(
                $additionalParams["reason"],
                $this->ship_reasons
            );
            unset($additionalParams["reason"]);
        }
        if (
            !isset($additionalParams["assurance.selection"]) ||
            $additionalParams["assurance.selection"] == ""
        ) {
            $additionalParams["assurance.selection"] = false;
        }
        $this->param = array_merge($this->param, $additionalParams);
        $this->setOptions(["action" => "api/v1/order"]);
        $this->setPost();
     ...
     */
    public function makeDoubleOrder($quot_info = [], $get_info = false)
    {
    ...
    private function switchPeople()
    {
        ...
        $old = ["expediteur", "destinataire", "tmp_exp", "tmp_dest"];
        $new = ["tmp_exp", "tmp_dest", "destinataire", "expediteur"];
