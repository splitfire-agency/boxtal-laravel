...
class ParcelPoint extends WebService
{
    ...
    const DROPOFF_POINT = "dropoff_point";
     ...
     * @var array
     */
    protected $types = [self::PICKUP_POINT, self::DROPOFF_POINT];
    ...
    public function getParcelPoint($type = "", $code = "", $country = "FR")
    {
        if (in_array($type, $this->types)) {
            ...
            $this->resp_errors_list[0] = [
                "code" => "type_not_correct",
                "url" => "",
            ];
        }
    }
    ...
    private function doSimpleRequest($type)
    {
        ...
        if ($source !== false) {
            ...
            if (count($this->resp_errors_list) == 0) {
                ...
                $point_detail = [
                    "code" => $this->xpath->query("./code", $point)->item(0)
                        ->nodeValue,
                    "name" => $this->xpath->query("./name", $point)->item(0)
                        ->nodeValue,
                    "address" => $this->xpath
                        ->query("./address", $point)
                        ->item(0)->nodeValue,
                    "city" => $this->xpath->query("./city", $point)->item(0)
                        ->nodeValue,
                    "zipcode" => $this->xpath
                        ->query("./zipcode", $point)
                        ->item(0)->nodeValue,
                    "country" => $this->xpath
                        ...
                        ->item(0)->nodeValue,
                    "latitude" => $this->xpath
                        ->query("./latitude", $point)
                        ->item(0)->nodeValue,
                    "longitude" => $this->xpath
                        ->query("./longitude", $point)
                        ->item(0)->nodeValue,
                    "phone" => $this->xpath->query("./phone", $point)->item(0)
                        ->nodeValue,
                    "description" => $this->xpath
                        ->query("./description", $point)
                        ->item(0)->nodeValue,
                ];
                ...
                $schedule = [];
                foreach (
                    $this->xpath->query("./schedule/day", $point)
                    as $d => $day_node
                ) {
                    $childs = $this->xpath->query("*", $day_node);
                    foreach ($childs as $child_node) {
                        if ($child_node->nodeName != "#text") {
                            $schedule[$d][$child_node->nodeName] =
