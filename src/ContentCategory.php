...
class ContentCategory extends WebService
{
    ...
    {
        $this->setOptions(["action" => "api/v1/content_categories"]);
        $this->doCatRequest();
    }
    ...
    {
        $this->setOptions(["action" => "api/v1/contents"]);
        $this->doConRequest();
    }
    ...
    private function doCatRequest()
    {
        ...
        if ($source !== false) {
            ...
            if (count($this->resp_errors_list) == 0) {
                ...
                $categories = $this->xpath->query(
                    "/content_categories/content_category"
                );
                foreach ($categories as $category) {
                    $code = $this->xpath->query("./code", $category)->item(0)
                        ->nodeValue;
                    $translationNodes = $this->xpath->query(
                    ...
                    foreach ($translationNodes as $translationNode) {
                        $translations[] = [
                            "locale" => $this->xpath
                                ->query("./locale", $translationNode)
                                ->item(0)->nodeValue,
                            "label" => $this->xpath
                                ->query("./label", $translationNode)
                                ->item(0)->nodeValue,
                        ];
                    }
        ...
        if ($source !== false) {
            ...
            if (count($this->resp_errors_list) == 0) {
                ...
                foreach ($contents as $content) {
                    ...
                    array_push($this->contents[$category_id], [
                        "code" => $this->xpath
                            ->query("./code", $content)
                            ->item(0)->nodeValue,
                        "label" => $this->xpath
