...
class Country extends WebService
{
    ...
    {
        $this->setOptions(["action" => "api/v1/countries"]);
        $this->doCtrRequest();
    }
    ...
    private function doCtrRequest()
    {
        ...
        if ($source !== false) {
            ...
            if (count($this->resp_errors_list) == 0) {
                ...
                $this->countries = [];
                foreach (
                    $this->xpath->query("/countries/country")
                    as $country
                ) {
                    $c = (object) [];
                    ...
                    $c->states = [];
                    foreach (
                        $this->xpath->query("./states/state", $country)
                        as $state
                    ) {
                        $c->states[] = (object) [
                            "code" => $this->xpath
    ...
    public function getCountry($code)
    {
        if (isset($this->countries[$code])) {
            $this->country = [0 => $this->countries[$code]];
            if (
                isset($this->codes_rel[$code]) &&
                $this->codes_rel[$code] != ""
            ) {
