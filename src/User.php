...
class User extends WebService
{
    ...
    {
        $this->setOptions(["action" => "api/v1/partnership"]);
        $this->setPartnership();
    }
    ...
    {
        $this->setOptions(["action" => "api/v1/emails_configuration"]);
            ...
            if ($mails) {
                foreach ($mails->childNodes as $config_line) {
                    if (!($config_line instanceof DOMText)) {
                        $this->user_configuration["emails"][
                            $config_line->nodeName
                        ] = $config_line->nodeValue;
                    }
                }
            }
        }
    ...
    {
        $this->setOptions(["action" => "api/v1/user_signup"]);
    ...
    {
        $this->setOptions(["action" => "api/v1/user_details"]);
        $this->setUserDetails();
        ...
        if ($source !== false) {
            ...
            if ($user) {
                foreach ($user->childNodes as $config_line) {
                    if (!($config_line instanceof DOMText)) {
                        if (
                            $config_line->hasChildNodes() &&
                            $config_line->childNodes->length > 1
                        ) {
                            foreach (
                                $config_line->childNodes
                                as $sub_config_line
                            ) {
                                if (!($sub_config_line instanceof DOMText)) {
                                    $this->user_configuration[
                                        $config_line->nodeName
                                    ][$sub_config_line->nodeName] =
                                        $sub_config_line->nodeValue;
                                }
                            }
                        } else {
                            $this->user_configuration[$config_line->nodeName] =
                                $config_line->nodeValue;
                        }
                    }
                }
    ...
    {
        $this->setOptions(["action" => "api/v1/user_keys"]);
        $this->param = array_merge($this->param, $params);
        $this->setPost();
        $source = parent::doRequest();
        if ($source !== false) {
