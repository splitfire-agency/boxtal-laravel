...
class ListPoints extends WebService
{
    ...
    public function getListPoints($carriers, $destination)
    {
        ...
        $this->setGetParams();
        $this->setOptions(["action" => "api/v1/listpoints"]);
        $this->doListRequest();
    }
        ...
        if ($source !== false) {
            ...
            if (count($this->resp_errors_list) == 0) {
                ...
                foreach ($carriers as $carrier_index => $carrier) {
                    $this->list_points[$carrier_index][
                        "operator"
                    ] = $this->xpath
                        ->query("./operator", $carrier)
                        ->item(0)->nodeValue;
                    $this->list_points[$carrier_index]["service"] = $this->xpath
                        ->query("./service", $carrier)
                        ->item(0)->nodeValue;
                    ...
                    foreach ($points as $point_index => $point) {
                        ...
                        $this->list_points[$carrier_index]["points"][
                            $point_index
                        ] = $point_info;
                    }
                }
